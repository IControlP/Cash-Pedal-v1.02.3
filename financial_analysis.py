...
class FinancialAnalysisService:
    ...
    def generate_financial_insights(self, affordability: Dict[str, Any], 
        ...
        if affordability['is_affordable']:
            insights.append(f"âœ… This vehicle fits well within your budget at {affordability['percentage_of_income']:.1f}% of income")
        else:
            insights.append(f"âš ï¸ This vehicle may strain your budget at {affordability['percentage_of_income']:.1f}% of income")
            insights.append(f"ðŸ’¡ Consider a vehicle costing ${affordability['over_budget_amount']:,.0f} less to stay within guidelines")
        
        # Cost optimization insights
        largest_category = max(cost_breakdown.items(), key=lambda x: x[1])
        insights.append(f"ðŸ’° {largest_category[0].replace('_', ' ').title()} is your largest cost at ${largest_category[1]:,.0f}")
