...
def display_vehicle_selection_form(display_mode: str = "collect") -> Dict[str, Any]:
    ...
    st.subheader("ï¿½ Vehicle Selection")
    ...
    if selected_make and selected_model and selected_year and selected_trim:
        ...
        with col1:
            ...
            if vehicle_age > 0:
                ...
                if avg_annual < 10000:
                    mileage_note = "ï¿½ Below average mileage"
                elif avg_annual < 15000:
                    mileage_note = "ï¿½ Average mileage"
                else:
                    mileage_note = "ï¿½ Above average mileage"
...
def display_location_form(vehicle_data: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    st.subheader("ï¿½ï¿½ Location & Regional Settings")
    ...
    if location_settings.get('is_set', False):
        st.success(f"ï¿½ Using saved location: {location_settings.get('zip_code', '')} - {location_settings.get('state', '')}")
        
        # Option to modify
        if st.button("ï¿½ Update Location Settings", key="update_location"):
    ...
    if st.session_state.get('show_location_form', True):
        ...
        with col1:
            ...
            if st.session_state.get('zip_code_valid', False):
                ...
                if validate_zip_code(zip_code):
                    ...
                    auto_fuel_price = 3.50
                    auto_electricity_rate = None
                    st.error("ï¿½ Invalid ZIP code format")
            else:
        ...
        if is_electric_vehicle:
            ...
            st.markdown("**ï¿½ Electric Vehicle Pricing**")
            ...
            if fuel_info:
                ...
                if requires_premium:
                    st.info(f"ï¿½ï¿½ï¿½ **Premium Fuel Required** for {make} {model} {trim if trim else ''}")
                    st.caption(f"Base regular: ${fuel_info.get('regular_price', 3.50):.2f}/gal ï¿½ Premium: ${detected_fuel_price:.2f}/gal (+$0.40)")
                else:
                    st.success(f"ï¿½ **Regular Fuel** for {make} {model}")
        ...
        if st.button(" Save Location Settings", key="save_location"):
            ...
            st.success("ï¿½ Location settings saved!")
            st.session_state.show_location_form = False
            st.rerun()
    else:
        # Use saved settings
        ...
        if make and model and selected_state and not is_electric_vehicle:
            ...
            if fuel_info.get('requires_premium', False):
                ...
                st.info(f"ï¿½ï¿½ï¿½ Premium fuel detected for {make} {model} - Price adjusted to ${fuel_price:.2f}/gal")
...
def display_insurance_form() -> Dict[str, Any]:
    ...
    st.subheader("ï¿½ï¸ Insurance Preferences")
    ...
    if insurance_settings.get('is_set', False):
        st.success(f"ï¿½ Using saved insurance: {insurance_settings.get('coverage_type', 'Full Coverage')}")
        
        col1, col2 = st.columns([1, 4])
        with col1:
            if st.session_state.show_insurance_form:
                if st.button("ï¿½ï¿½ï¸ Hide", key="hide_insurance"):
                    st.session_state.show_insurance_form = False
                    st.rerun()
            else:
                if st.button("ï¿½ Update", key="show_insurance"):
    ...
    if st.session_state.get('show_insurance_form', True):
        ...
        if st.button(" Save Insurance Settings", key="save_insurance"):
            ...
            st.success("ï¿½ Insurance settings saved!")
            st.session_state.show_insurance_form = False
            st.rerun()
    else:
...
def display_financial_parameters_form(transaction_type: str) -> Dict[str, Any]:
    ...
    st.subheader("ï¿½ Financial Parameters")
    ...
    if transaction_type == "Purchase":
        ...
        if payment_method == "Cash Purchase":
            ...
            st.info("ï¿½ï¿½ï¿½ **Loan Financing Selected**")
...
def display_analysis_settings_form(transaction_type: str = "Purchase") -> Dict[str, Any]:
    ...
    st.subheader("ï¿½ Analysis Settings")
    ...
    with col2:
        ...
        if st.button(" Save Analysis Preferences", key="save_analysis"):
            ...
            st.success("ï¿½ Analysis preferences saved!")
...
def display_settings_management_sidebar():
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("ï¿½ï¸ Persistent Settings")
    ...
    if get_persistent_setting('location', 'is_set', False):
        saved_settings.append("ï¿½ï¿½ Location")
    if get_persistent_setting('personal', 'is_set', False):
        saved_settings.append("ï¿½ Personal Info")
    if get_persistent_setting('insurance', 'is_set', False):
        saved_settings.append("ï¿½ï¸ Insurance")
    if get_persistent_setting('analysis', 'is_set', False):
        saved_settings.append("ï¿½ Analysis Prefs")
    
    if saved_settings:
        st.sidebar.success(f"ï¿½ Saved: {', '.join(saved_settings)}")
    else:
    ...
    with col1:
        if st.button("ï¿½ Reset All", key="reset_all_settings", help="Clear all saved settings"):
    ...
    with col2:
        ...
        if st.button("ï¿½ View All", key="view_all_settings", help="View all saved settings"):
...
def display_settings_summary():
    ...
    if st.session_state.get('show_settings_summary', False):
        with st.expander("ï¿½ Current Saved Settings", expanded=True):
            initialize_persistent_settings()
            
            # Location settings
            location = get_persistent_setting('location')
            if location.get('is_set', False):
                st.write("**ï¿½ï¿½ Location & Regional:**")
            ...
            if personal.get('is_set', False):
                st.write("**ï¿½ Personal Information:**")
            ...
            if insurance.get('is_set', False):
                st.write("**ï¿½ï¸ Insurance Settings:**")
            ...
            if analysis.get('is_set', False):
                st.write("**ï¿½ Analysis Preferences:**")
            ...
            if st.button("ï¿½ Close", key="close_settings_summary"):
...
def display_all_forms_visible() -> Tuple[Dict[str, Any], bool, str]:
    ...
    st.markdown("## ï¿½ Vehicle Analysis Form")
...
def get_default_form_data(vehicle_override: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    base_data = {
        ...
        'driving_style': 'normal',  # ï¿½ï¿½ ADDED: Default driving style
        'terrain': 'flat'  # ï¿½ï¿½ ADDED: Default terrain
    }
    ...
    # Add personal data if saved - ï¿½ï¿½ FIXED: Now includes driving_style and terrain
    personal = get_persistent_setting('personal')
    if personal.get('is_set', False):
        base_data.update({
            'user_age': personal.get('user_age', 35),
            ...
            'annual_mileage': personal.get('annual_mileage', 12000),  # ï¿½ï¿½ ADDED
            'driving_style': personal.get('driving_style', 'normal'),  # ï¿½ï¿½ ADDED
            'terrain': personal.get('terrain', 'flat'),  # ï¿½ï¿½ ADDED
            'num_household_vehicles': personal.get('num_household_vehicles', 2)  # ï¿½ï¿½ ADDED
        })
