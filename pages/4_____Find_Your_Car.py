...
def get_slider_label(value):
 ...
 return labels.get(value, "Sometimes?  ’Â¡Â¢")
...
def display_result_card(vehicle_type, score, rank):
 ...
  ’Â¡ƒ  #{rank} Match: {profile.get('emoji', ' ’Â¦ƒƒ')} {profile.get('name', vehicle_type.title())}
 </h2>
 ...
 #{rank}: {profile.get('emoji', ' ’Â¦ƒƒ')} {profile.get('name', vehicle_type.title())}
 </h3>
...
def main():
 ...
 <p class="quiz-title"> ’Â¦Â¢ Find Your Perfect Car</p>
 ...
 # Sidebar
 with st.sidebar:
 st.header(" ’Â¦Â¢ Quiz Progress")
 
 if st.session_state.quiz_complete:
 st.success(" ’¢™¡ Quiz Complete!")
 if st.button(" ’’‚Â¢¬ Retake Quiz"):
 st.session_state.quiz_started = False
 st.session_state.quiz_complete = False
 st.session_state.quiz_answers = {}
 st.rerun()
 ...
 *Shorter than your last doom scroll*
 ...
 if st.button(" ’Â¦Â¢ Let's Do This!", use_container_width=True, type="primary"):
 ...
 <h1 style="margin:0;"> ’Â¦ƒ The Results Are In!</h1>
 ...
 with col2:
 st.markdown("####  ’Â¡ƒ Consider:")
 for item in profile.get("consider", []):
 st.markdown(f"- {item}")
 
 st.markdown("####  ’Â¦ƒƒ Top Picks to Explore:")
 ...
 # Call to action
 st.markdown("---")
 st.markdown("###  ’Â¦ƒ Ready to Explore Your Matches?")
 ...
 with col2:
 if st.button(" ’’ ’Â¡ Compare Vehicles", use_container_width=True):
 st.switch_page("pages/2____Multi_Vehicle_Comparison.py")
 
 with col3:
 if st.button(" ’’‚Â¢¬ Retake Quiz", use_container_width=True):
 st.session_state.quiz_started = False
 st.session_state.quiz_complete = False
 st.session_state.quiz_answers = {}
 st.rerun()
