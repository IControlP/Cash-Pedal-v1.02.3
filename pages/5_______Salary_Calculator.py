...
def display_maintenance_breakdown(maintenance: dict, vehicle_tier: str):
    ...
    <div style="background-color: #f5f5f5; padding: 20px; border-radius: 10px; margin-top: 20px;">
        <h4 style="margin-top: 0;">Ã°Å¸â€Â§ Estimated Annual Maintenance Costs</h4>
...
def main():
    ...
    if is_lease:
        st.caption("Ã°Å¸â€œÅ’ Showing 2025 model year vehicles only (current year for leasing)")
        available_year = 2025
    else:
        st.caption("Ã°Å¸â€œÅ’ Select any available model year")
    ...
    if VEHICLE_DATABASE_AVAILABLE:
        ...
        if make and model and make != "-- Select Make --" and model != "-- Select Model --":
            ...
            tier_labels = {
                'luxury': 'Ã°Å¸Ââ€  Luxury',
                'premium': 'Ã¢Â­Â Premium', 
            ...
            if is_used_vehicle and estimated_value:
                st.success(f"**{year} {make} {model} {trim}** Ã¢â‚¬â€ Est. Value: {format_currency(vehicle_price)} (MSRP: {format_currency(original_msrp)}) Ã¢â‚¬â€ Category: {tier_labels.get(vehicle_tier, 'Standard')}")
                st.caption(f"Ã°Å¸â€œâ€° Estimated value based on {vehicle_age} years of age and ~{estimated_mileage:,} miles (average use)")
            else:
                st.success(f"**{year} {make} {model} {trim}** Ã¢â‚¬â€ MSRP: {format_currency(vehicle_price)} Ã¢â‚¬â€ Category: {tier_labels.get(vehicle_tier, 'Standard')}")
    else:
    ...
    st.caption("Ã°Å¸â€œÅ’ Your state affects income tax calculations")
    ...
    if not state_info[2]:  # No income tax
        st.info(f"Ã¢Å“Â¨ {selected_state_name} has **no state income tax** Ã¢â‚¬â€ more of your paycheck stays with you!")
    elif state_info[0] >= 0.07:
        st.warning(f"Ã°Å¸â€œÅ  {selected_state_name} has a higher state income tax rate ({state_info[0]*100:.1f}%)")
    ...
    if not is_lease:
        ...
        st.caption("Ã°Å¸â€œÅ’ A larger down payment reduces your monthly payment and required salary")
        ...
        if make and model and make != "-- Select Make --" and model != "-- Select Model --":
            ...
            if down_payment_percent >= 50:
                ...
                st.warning("Ã¢Å¡Â Ã¯Â¸Â Financing 100% of the vehicle will result in higher monthly payments and more interest paid.")
        else:
            st.info("Ã°Å¸â€˜â€  Select a vehicle first to set your down payment.")
    ...
    if make and model and make != "-- Select Make --" and model != "-- Select Model --":
        ...
        <div style="background-color: #e8f5e9; padding: 25px; border-radius: 10px; text-align: center; border: 2px solid #4caf50;">
            <h3 style="color: #2e7d32; margin-top: 0;">Ã°Å¸â€œÅ  Want More Details?</h3>
        ...
        with col2:
            if st.button("Ã°Å¸â€Â§ Try the Full Vehicle Cost Calculator", type="primary", use_container_width=True):
                st.switch_page("pages/1___Single_Vehicle_Calculator.py")
    
    else:
        # Prompt to select vehicle
        st.info("Ã°Å¸â€˜â€  Please select a vehicle above to see salary requirements.")
