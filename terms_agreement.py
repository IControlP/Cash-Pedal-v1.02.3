...
def get_database_url() -> Optional[str]:
    ...
    if 'host=/var/run/postgresql' in db_url or '.s.PGSQL' in db_url:
        ...
        if match:
            ...
            print(f"ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Converted socket-based URL to TCP/IP")
...
def show_terms_fullscreen():
    ...
    <div class="terms-header">
        <h1>[icon] Welcome to CashPedal</h1>
    ...
    with st.expander("[icon] View Full Terms and Conditions (Click to Expand)", expanded=False):
        st.markdown(TERMS_AND_CONDITIONS)
    
    st.markdown("---")
    
    # Required acknowledgments
    st.markdown("### âœ… Required Acknowledgments")
    ...
    if not all_checked:
        st.warning("âš ï¸ Please check all three boxes above to accept the terms and continue.")
    ...
    with col2:
        if st.button(
            "âœ… I Accept These Terms - Continue to CashPedal", 
            type="primary",
            ...
            if record_id:
                ...
                st.success("âœ… Terms accepted! Loading application...")
                st.rerun()
            else:
...
def show_database_debug_info():
    ...
    if private_url and private_url != '' and private_url != '<empty string>':
        st.success("ÃƒÆ’Ã‚Â¢Ãƒâ€¦ **DATABASE_PRIVATE_URL:** Set (preferred for Railway)")
        st.code(private_url[:80] + "...", language="text")
    elif 'DATABASE_PRIVATE_URL' in os.environ:
        st.error("ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦ **DATABASE_PRIVATE_URL:** Empty string (variable exists but has no value)")
        st.warning("**FIX:** Delete this variable and add it manually with the actual PostgreSQL URL")
    else:
        st.warning("ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â **DATABASE_PRIVATE_URL:** Not set")
    
    # Check DATABASE_URL
    if db_url and db_url != '' and db_url != '<empty string>':
        if 'socket' in db_url or '/var/run/postgresql' in db_url:
            st.error("ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦ **DATABASE_URL:** Socket-based (won't work in containers)")
            st.code(db_url[:80] + "...", language="text")
        else:
            st.info("ÃƒÆ’Ã‚Â¢¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â **DATABASE_URL:** Set")
            st.code(db_url[:80] + "...", language="text")
    elif 'DATABASE_URL' in os.environ:
        st.error("ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦ **DATABASE_URL:** Empty string (variable exists but has no value)")
        st.warning("**FIX:** Delete this variable and add it manually with the actual PostgreSQL URL")
    else:
        st.error("ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦ **DATABASE_URL:** Not set")
    ...
    if final_url:
        ...
        if private_url and final_url == private_url:
            st.success("ÃƒÆ’Ã‚Â¢Ãƒâ€¦ Using **DATABASE_PRIVATE_URL** (best for Railway)")
        elif 'railway.internal' in final_url:
            st.info("ÃƒÆ’Ã‚Â¢¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Using **converted TCP/IP URL** (socket URL was converted)")
        else:
            st.info("ÃƒÆ’Ã‚Â¢¹ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Using **DATABASE_URL**")
        
        st.code(final_url[:80] + "...", language="text")
    else:
        st.error("ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦ **No valid database URL found!**")
        st.write("---")
        st.write("### ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Â§ How to Fix This")
        st.markdown("""
        ...
        **Solution:**
        ...
        3. Find `DATABASE_PRIVATE_URL` and click ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â to reveal
        4. **Copy the entire URL**
        5. Go to your **Streamlit service** (this service)
        6. Variables tab ÃƒÆ’Ã‚Â¢¢ Remove the empty `DATABASE_PRIVATE_URL`
    ...
    if is_postgres_available():
        ...
        try:
            ...
            st.success(f"ÃƒÆ’Ã‚Â¢Ãƒâ€¦ PostgreSQL connection successful!")
            ...
            if table_exists:
                st.success("ÃƒÆ’Ã‚Â¢Ãƒâ€¦ Table 'consent_records' exists")
                ...
                st.warning("ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Table 'consent_records' does not exist - will be created on first use")
            
            conn.close()
            
        except Exception as e:
            st.error(f"ÃƒÆ’Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€¦ PostgreSQL connection failed!")
        ...
        st.warning("ÃƒÆ’Ã‚Â¢Ãƒâ€¦Ã‚Â¡Ãƒâ€šÃ‚Â ÃƒÆ’Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â PostgreSQL not configured - using JSON fallback")
